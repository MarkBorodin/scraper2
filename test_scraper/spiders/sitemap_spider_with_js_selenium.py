...
class MySpider(SitemapSpider):
    ...
    def _parse_sitemap(self, response):
        ...
        pool = ThreadPool()
        for s in pool.map(self.get_soup, urls_list):
            yield self.parse(s)
        pool.close()
        pool.join()
    ...
    def parse(self, soup_and_url, **kwargs):
        """parse page, page data and write to db"""

        # create item
        item = Subpage()

        soup, url = soup_and_url
    ...
    @staticmethod
    def get_soup(url):
        """start web browser with selenium and get soup. Returns a tuple: url and soup"""
        try:
